...
samples <- metropolis_hasting(x0, N, s)

# visualizing the results
ggplot() +
  geom_histogram(aes(x = samples), bins = 50, fill = "blue", alpha = 0.5, density = TRUE) +
  stat_function(fun = function(x) f(x), color = "red", size = 1.2) +
...
```{r}
# setting up variable and parambeter for chains
J <- 9 ## no. of chains
x0j <- runif(J) ## random initial value between 1 and J

# creating J number of chains
chain <- metropolis_hasting(x0j[J], N, s)
...
# listing of all years (1998L2007)
years <- 2003:2004
...
plane_age_query <- "
  ...
  FROM
    planes
  JOIN master
    ON planes.tailNum = master.Tailnum
  WHERE 
    Age >0 AND
    Age IS NOT NULL AND
    planes.year IS NOT NULL AND
    planes.year NOT LIKE '%N%'
"
